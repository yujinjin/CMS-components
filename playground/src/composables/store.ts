...
import CMSComponentsCode from "../template/cms-components.ts?raw";
...
export default function (serializedState?: string) {
    ...
    const CMS_COMPONENTS_FILE = "src/cms-components.ts";
    ...
    state.builtinImportMap = computed<ImportMap>(() => {
        return {
            imports: {
                ...
                "@yujinjin/cms-components": `https://fastly.jsdelivr.net/npm/@yujinjin/cms-components@${state.dependencyVersion.value.CMSComponents}/dist/index.full.min.mjs`,
    ...
    const init = async function () {
        ...
        watch(
            () => state.dependencyVersion.value.CMSComponents,
            () => {
                const style = `https://fastly.jsdelivr.net/npm/@yujinjin/cms-components@${state.dependencyVersion.value.CMSComponents}/style/index.css`;

                const file = new File(CMS_COMPONENTS_FILE, CMSComponentsCode.replace("#STYLE#", style).trim(), true);
                state.files.value[CMS_COMPONENTS_FILE] = file;
                compileFiles([file]);
            },
            {
                immediate: true
    ...
    const deserialize = function (serializedState: string) {
        ...
        try {
            ...
            if (Object.prototype.hasOwnProperty.bind(files)("_CMS-Components_version")) {
                state.dependencyVersion.value.CMSComponents = files["_CMS-Components_version"] || state.dependencyVersion.value.CMSComponents;
                delete files["_CMS-Components_version"];
            }
            setFiles(files);
        } catch (error) {
            console.error(error);
    ...
    const initFiles = function (serializedState?: string) {
        ...
        if (serializedState) {
            ...
            state.files.value[CMS_COMPONENTS_FILE] = new File(CMS_COMPONENTS_FILE, CMSComponentsCode, true);
    ...
    const renameFile = function (oldFilename: string, newFilename: string) {
        ...
        if (file.hidden || [APP_FILE, MAIN_FILE, ELEMENT_PLUS_FILE, CMS_COMPONENTS_FILE, IMPORT_MAP].includes(oldFilename)) {
    ...
    const deleteFile = async function (filename: string) {
        if ([ELEMENT_PLUS_FILE, CMS_COMPONENTS_FILE, MAIN_FILE, APP_FILE, IMPORT_MAP].includes(filename)) {
    ...
    const setFiles = async function (newFiles: Record<string, string>, mainFile = store.mainFile) {
        ...
        if (files[CMS_COMPONENTS_FILE]) {
            files[CMS_COMPONENTS_FILE].hidden = true;
        } else {
            files[CMS_COMPONENTS_FILE] = state.files.value[CMS_COMPONENTS_FILE] || new File(CMS_COMPONENTS_FILE, CMSComponentsCode, true);
        }
        if (!files[IMPORT_MAP]) {
