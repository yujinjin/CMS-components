...
import { BUILD_ES_ROOT, BUILD_ROOT, BUILD_LIB_ROOT, PACKAGES_ROOT, COMPONENTS_ROOT, COMPONENTS_MAIN_ROOT } from "../utils/constants";
export default async function () {
    console.info("开始-------------单个组件打包 esm模块和cjs模块");

    const packageJson = JSON.parse(fs.readFileSync(resolve(BUILD_ROOT, "package.json"), "utf8"));

    const getInputs = async function (rootDir: string) {
        const files: string[] = await fastGlob("**/*.{js,ts,vue}", {
            cwd: rootDir,
            absolute: true,
            onlyFiles: true,
            ignore: ["node_modules", "__tests__/**/*", "vitest.config.ts"]
        });
        // 添加main目录下的文件
        return files;
    };
    ...
    const [componentsInputFiles, componentsMainInputFiles] = await Promise.all([getInputs(COMPONENTS_ROOT), getInputs(COMPONENTS_MAIN_ROOT)]);
